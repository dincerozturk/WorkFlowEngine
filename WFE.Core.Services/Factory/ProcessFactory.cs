...
namespace WorkFlowManager.Common.Factory
{
    ...
    {
        public Process CreateProcess(WorkFlowManager.Common.Tables.Task task, string name, ProjectRole assignedRole, string description = null, FormView formView = null)
        {
            return new Process(task, name, assignedRole, description, formView);
        }

        public SubProcess CreateSubProcess(WorkFlowManager.Common.Tables.Task task, string name, List<TaskVariable> taskVariableList)
        {
            return new SubProcess(task, name, JsonConvert.SerializeObject(taskVariableList));
        }


        public Condition CreateCondition(WorkFlowManager.Common.Tables.Task task, string name, ProjectRole assignedRole, string variableName = null, string description = null, FormView formView = null)
        {
            return new Condition(task, name, assignedRole, variableName, description, formView);
        }
        public DecisionPoint CreateDecisionPoint(WorkFlowManager.Common.Tables.Task task, string name, DecisionMethod decisionMethod, string variableName = null, int repetitionFrequenceByHour = 1, string description = null, FormView formView = null)
        {
