...
namespace WorkFlowManager.Common.Extensions
{
    ...
    {
        public static HtmlString ShowLabel(this IHtmlHelper htmlHelper, string label)
        {
            if (label != null)
            {
        ...
        public static IActionResult WithMessage(this IActionResult viewResult, Controller controller, string message, MessageType messageType = MessageType.Success)
        {
            controller.ShowMessage(message, messageType);
            return viewResult;
        }

        public static IActionResult WithMessage(this RedirectToRouteResult viewResult, Controller controller, string message, MessageType messageType = MessageType.Success)
        {
            controller.ShowMessage(message, messageType);
            return viewResult;
        }
            ...
            foreach (var messageType in Enum.GetNames(typeof(MessageType)))
            {
                ...
                if (message != null)
                {
                    ...
                    if (type == MessageDialogType.BootStrap)
                    {
                        ...
                        messageBoxBuilder.InnerHtml.AppendHtml(string.Format("<button class='close' data-dismiss='alert' aria-label='close'>&times;</button><strong>{0}</strong> {1}", messageTitle, message.ToString()));
                    }
                    else
                    {
                        messageBoxBuilder.AddCssClass(string.Format("alert alert-{0} fade in", lowermessage));
                        ...
                        messageBoxBuilder.InnerHtml.AppendHtml(string.Format("<button type = 'button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>" +
                                          "<h4><i class='icon fa {0}'></i> {1}</h4>{2}", msgicon, messageTitle, message.ToString()));
                    }
                    messages += messageBoxBuilder.ToString();
                }
            }
            var htmlString = new HtmlString(messages);
            return htmlString;
