...
namespace WorkFlowManager.Web.Controllers
{
    public class WorkFlowController : Controller
    {
        ...
        public IActionResult Save(ProcessForm formData)
        {
        ...
        public IActionResult SaveClose(ProcessForm formData)
        {
            if (SaveProcessForm(ref formData))
            {
        ...
        [HttpPost, ValidateAntiForgeryToken]
        public IActionResult Delete(int processId, int taskId)
        {
            try
            {
        ...
        public IActionResult EditByProcessCode(string processCode)
        {
        ...
        public IActionResult SetNext(string taskId, string processCode, int? nextId)
        {
            try
            {
        ...
        public void ProcessFormInitialize(ref ProcessForm formData)
        {
            ...
            formData.MainProcessList = mainProcessList;// (mainProcessList != null ? new CustomSelectList(mainProcessList, "Id", "Name") : null);

            if (formData.ProcessType == ProcessType.DecisionPoint)
            {
                formData.DecisionMethodList = _workFlowService.GetDecisionMethodList(formData.TaskId);// new CustomSelectList(_workFlowService.GetDecisionMethodList(formData.TaskId), "Id", "MethodName");
                formData.RepetitionHourList = Enumerable.Range(1, 24);// new CustomSelectList(Enumerable.Range(1, 24));
            }
            else if (formData.ProcessType == ProcessType.Process)
            {
                formData.FormViewList = _workFlowService.GetFormViewList(formData.TaskId);// new CustomSelectList(_workFlowService.GetFormViewList(formData.TaskId), "Id", "FormName");
            }
        ...
        public IActionResult New(int taskId, ProcessType processType = ProcessType.Process, int conditionId = 0)
        {
        ...
        [HttpGet]
        public IActionResult WorkFlow(int taskId = 0)
        {
        ...
        //[ChildActionOnly]
        public IActionResult SpecialForm(int taskId)
        {
        ...
        //[ChildActionOnly]
        public IActionResult DecisionMethod(int taskId)
        {
        ...
        //[ChildActionOnly]
        public IActionResult WorkFlowSummary(int taskId)
        {
            return PartialView("_WorkFlowSummary", _workFlowService.GetWorkFlowSummary(taskId));
        }

        //[ChildActionOnly]
        public IActionResult ShowWorkFlow(int taskId)
        {
