...
IMapper mapper)
        ...
        public int StartWorkFlow(int ownerId, WorkFlowManager.Common.Tables.Task task)
        {
        ...
        {
            var task = _unitOfWork.Repository<WorkFlowManager.Common.Tables.Task>().GetAll().Where(x => x.Name == taskName).FirstOrDefault();
            return StartWorkFlow(ownerId, task);
        }

        public int StartWorkFlow(int ownerId, int taskId)
        {
            var task = _unitOfWork.Repository<WorkFlowManager.Common.Tables.Task>().Get(taskId);
            return StartWorkFlow(ownerId, task);
        }
        ...
        private void CreateNewWorkFlowTrace(Process targetProcess, int ownerId)
        {
            ...
            else if (targetProcess is SubProcess)
            {
                ...
                foreach (var taskVariable in taskVariableList)
                {
                    ...
                    var task = _unitOfWork.Repository<WorkFlowManager.Common.Tables.Task>().Get(taskVariable.TaskId);
